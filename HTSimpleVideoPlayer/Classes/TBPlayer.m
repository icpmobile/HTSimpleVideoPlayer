...
+ (instancetype)sharedInstance {
    // 单个视频重复播放，就是播放界面频繁退出返回，单例可能会造成视频无法播放
...
- (void)resumeOrPause
{
    ...
    if (self.state == TBPlayerStatePlaying) {
        ...
        [self.player play];
        self.state = TBPlayerStatePlaying;
    } else if (self.state == TBPlayerStateStopped) {
        [_stopButton setImage:[self imageNamed:@"icon_pause"] forState:UIControlStateNormal];
        [_stopButton setImage:[self imageNamed:@"icon_pause_hl"] forState:UIControlStateHighlighted];
        [self rePlayVideo];
        self.state = TBPlayerStatePlaying;
...
- (UIImage *)imageNamed:(NSString *)imageName {
    ...
    NSString *imgHeader = [imageName stringByAppendingString:@"@"];
    NSString *imgFooter = [NSString stringWithFormat:@"%ldx.png", scale];
    NSString *imgNameStr = [imgHeader stringByAppendingString:imgFooter];
    
    return [currentBundle pathForResource:imgNameStr ofType:nil inDirectory:@"HTSimpleVideoPlayer.bundle"];
}
